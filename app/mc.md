...
!> MossFrp 不提供 Minecraft 服务器相关帮助，文档主要由热心网友提供  
...
!> 如果您的服务器经常被 [DOS 攻击](https://zh.wikipedia.org/wiki/拒绝服务攻击)，请使用高防节点
...
如果您有 **其他厂商** 的域名，并且会在对应面板中进行 **SRV解析**，欢迎提交 [Pull Request](https://github.com/Hua-Fan-Yang/MossFrpWiki/pulls ':target=_blank') 帮助我们完善该文档
...
| 记录值 | `0 5 远程端口 隧道域名` (例: `0 5 xxxxx xxx.mossfrp.cn`) |

![](./_images/mc-srv-3.png)

保存完毕后需要 **等待** 10 分钟 (按照您的 **TTL** 来决定，一般为 10 分钟) 

+ **假设** 您的域名为: mossfrp.cn，您的 **xx** 设置为 `test`，那么使用 `mossfrp.cn` 即可连接到您的服务器
+ **假设** 您的域名为: mossfrp.cn，您把 **.xx** 去掉了，那么使用 `mossfrp.cn` 即可连接到您的服务器
...
| 记录类型 | SRV |
| 主机记录 | `_minecraft._tcp.xx` (后面的 `xx` 可以自定义，也可以直接把 `.xx` 去掉只写 `_minecraft._tcp`) |
| 记录值 | `0 5 远程端口 隧道域名` (例: `0 5 xxxxx xx.mossfrp.cn`) |

保存完毕后需要 **等待** 10 分钟来让解析生效 (按照您的 **TTL** 来决定，一般为 10 分钟) 。

+ **假设** 您的域名为: mossfrp.cn，您的 **`.xx`** 设置为 `test`，那么使用 `test.mossfrp.cn` 即可连接到您的服务器
+ **假设** 您的域名为: mossfrp.cn，您把 **`.xx`** 去掉了，那么使用 `mossfrp.cn` 即可连接到您的服务器
...
| 权重 | 5 |
| 端口 | 远程端口 (例: `12345`) |
| 目标 | 隧道域名 (例: `xxx.mossfrp.cn`) |

![](./_images/mc-srv-6.png)

保存完毕后需要 **等待** 10 分钟 (按照您的 **TTL** 来决定，一般为 10 分钟) 

+ 假设您的域名为: mossfrp.cn，您的 **名称** 设置为 `test`，那么使用 `test.mossfrp.cn` 即可连接到您的服务器
+ 假设您的域名为: mossfrp.cn，您的 **名称** 设置为 `@`，那么使用 `mossfrp.cn` 即可连接到您的服务器
